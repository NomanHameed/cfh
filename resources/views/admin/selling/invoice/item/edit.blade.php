<div id="editItem{{$item->id}}" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('invoice.items.update', $item->id) }}" class="validate{{$item->id}}" role="form" enctype="multipart/form-data">
                @csrf
                {{ method_field('PATCH') }}
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="form-group mb-3">
                            {{ Form::label('item') }}
                            {{ Form::select('sale_item_id', saleItems(), $item->sale_item_id, ['class' => 'form-control form-select' . ($errors->has('sale_item_id') ? ' is-invalid' : ''), 'placeholder' => '--Select--','required']) }}
