...
<div class="col-md-12">
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">{{ __('Edit ') }} Transfer </h5>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('transfers.update', $transfer->id) }}" class="validate"   role="form" enctype="multipart/form-data">
                @csrf
                {{ method_field('PATCH') }}
                 @include('admin.banking.transfer.form')
            </form>
        </div>
    </div>
</div>
